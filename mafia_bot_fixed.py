"""Mafia 42 — بوت ديسكورد متكامل النسخة 3.1 (إصلاح شامل)

الإصلاحات:
- منطق check_winner مُصلَح: لا يعدّ القاتل مافيا حتى ينضم فعلاً
- distribute_roles مُصلَح: عدد المافيا صحيح لكل الأحجام
- save_game_to_history مُعرَّفة
- الممرضة ونائب الشرطي يرثان فقط عند الموت في نفس الليلة
- المحرّض مُصلَح في التصويت
- _update_player_stats_after_game تحسب الفائز بشكل صحيح
- check_winner لا يفوز إذا لم يتبقَ لاعبون
- إضافة تأخير صغير بعد توزيع الأدوار قبل أول check_winner
"""
...
ROLES: dict[str, Role] = {
    "مافيا": Role("مافيا", "mafia", "يقتل لاعباً كل ليلة بالتنسيق مع باقي المافيا.", "🔪", True, "common", "نسّق مع زملائك في المافيا وتجنّب اختيار هدف محروس.", "فوز المافيا عند تساويهم مع المواطنين."),
    "رئيس المافيا": Role("رئيس المافيا", "mafia", "يقتل كل ليلة. إذا مات، يختار وريثاً سرياً من بقية المافيا يأخذ دور 'مافيا' جديد.", "👑", True, "legendary", "اختر الوريث بحكمة — الأقدر على الاختباء.", "فوز المافيا عند تساويهم مع المواطنين."),
    "وحش": Role("وحش", "mafia", "يبحث عن المافيا كل ليلة. إذا وقع اختياره على مافيا، تخترق ضربتهم حماية الطبيب والجندي.", "👹", True, "rare", "ابحث عن لاعب تعتقد أنه مواطن لكشف القوة.", "فوز المافيا."),
    "مضيفة": Role("مضيفة", "mafia", "تختار لاعباً وتمنعه من تنفيذ دوره الليلة.", "💋", True, "common", "امنعي الشرطي أو الطبيب في الليالي الحساسة.", "فوز المافيا."),
    "ساحرة": Role("ساحرة", "mafia", "تسحر لاعباً كل ليلة فتمنع قدرته (لا يعمل سحرها على المافيا).", "🧙‍♀️", True, "rare", "الفرق بينها وبين المضيفة: سحرها لا يؤثر على المافيا.", "فوز المافيا."),
    "محتال": Role("محتال", "mafia", "يظهر بريئاً عند تحقيق الشرطي معه.", "🎭", False, "rare", "كن أكثر هجومية في النقاش — أنت بريء رسمياً.", "فوز المافيا."),
    "جاسوسة": Role("جاسوسة", "mafia", "تكشف هوية لاعب كل ليلة، وتشاركها مع باقي المافيا.", "🕵️‍♀️", True, "common", "ابدئي بالأدوار الخطرة كالشرطي والحارسة.", "فوز المافيا."),
    "مزوّر": Role("مزوّر", "mafia", "كل ليلة يختار لاعباً ويغيّر الدور الذي سيظهر للعرافة أو المراسلة عنه.", "🖊️", True, "legendary", "استخدمه ضد العرافة والشرطي لبث الفوضى.", "فوز المافيا."),
    "مخبر": Role("مخبر", "mafia", "يصطنع أدلة كاذبة — يجعل نتيجة تحقيق الشرطي عكس الحقيقة لليلة واحدة.", "📋", True, "legendary", "استخدمه ضد الشرطي لإضلاله عن أحد زملائك.", "فوز المافيا."),
    "محرّض": Role("محرّض", "mafia", "يختار لاعباً في الليل فيُجبَر على التصويت ضد هدف المحرّض في النهار.", "📢", True, "legendary", "استخدمه لتوجيه الإعدام نحو لاعب بريء قوي.", "فوز المافيا."),
    "عنكبوت": Role("عنكبوت", "mafia", "ينصب فخاً على لاعب: إذا استهدفه أي دور ليلاً، يكشف العنكبوت هويته.", "🕷️", True, "legendary", "ضع الفخ على نفسك أو على لاعب تعتقد أن الشرطي سيحقق معه.", "فوز المافيا."),
    "شرطي": Role("شرطي", "citizens", "يحقق مع لاعب ليكشف هل هو مافيا أم لا. المحتال يبدو بريئاً.", "🚓", True, "common", "لا تكشف نتائجك مبكراً — قد تُستهدف في الليل.", "فوز المواطنين."),
    "طبيب": Role("طبيب", "citizens", "يحمي لاعباً من القتل (يستطيع حماية نفسه).", "💉", True, "common", "حاول عدم حماية نفسك كل ليلة — وزّع الحماية.", "فوز المواطنين."),
    "حارسة": Role("حارسة", "citizens", "تحرس لاعباً — إذا هاجمته المافيا، تقتل المهاجم.", "🛡️", True, "rare", "لا تحرسي الشرطي في كل ليلة — المافيا ستتجنبه.", "فوز المواطنين."),
    "جندي": Role("جندي", "citizens", "ينجو من أول هجوم عليه (مرة واحدة فقط).", "💂", False, "common", "استخدم بقاءك لتصويت قوي في النهار.", "فوز المواطنين."),
    "ممرضة": Role("ممرضة", "citizens", "ترث دور الطبيب تلقائياً إذا مات.", "🏥", False, "common", "ابقَ هادئاً — ستكشف قيمتك عند موت الطبيب.", "فوز المواطنين."),
    "عميل سري": Role("عميل سري", "citizens", "ابتداءً من الليلة الثانية، يكشف دور مواطن عشوائي كل ليلة.", "🕴️", True, "rare", "استخدم معلوماتك بذكاء — لا تكشفها بسرعة.", "فوز المواطنين."),
    "كاهن": Role("كاهن", "citizens", "يعيد لاعباً ميتاً للحياة — مرة واحدة فقط.", "⛪", True, "rare", "أعِد لاعباً ذا دور قوي كالشرطي أو الحارسة.", "فوز المواطنين."),
    "عرافة": Role("عرافة", "citizens", "تكشف دور لاعب ميت كل ليلة.", "🔮", True, "rare", "ابدئي بأكثر المشبوهين وفاةً.", "فوز المواطنين."),
    "مراسلة": Role("مراسلة", "citizens", "تنشر دور لاعب علناً في الصباح — مرة واحدة فقط.", "📰", True, "common", "استخدمي القدرة في اللحظة المناسبة — ليس مبكراً جداً.", "فوز المواطنين."),
    "شهيد": Role("شهيد", "citizens", "إذا أُعدم أو قُتل ليلاً، يأخذ معه أحداً (أول من صوّت ضده، أو قاتله الليلي).", "💀", False, "rare", "دعهم يصوّتون ضدك — ثم تفاجأ بالانتقام.", "فوز المواطنين."),
    "رجل عصابة": Role("رجل عصابة", "citizens", "يمنع لاعباً من التصويت في النهار التالي.", "🚫", True, "common", "امنع أكثر المصوّتين ضد المواطنين.", "فوز المواطنين."),
    "سياسي": Role("سياسي", "citizens", "صوته يُحسب مرتين، ولا يمكن إعدامه بالتصويت — يكشف البوت أنه شخص عادي.", "🎩", False, "rare", "استغل تأثيرك في المواقف الحاسمة.", "فوز المواطنين."),
    "مواطن": Role("مواطن", "citizens", "لا قدرات خاصة. يصوت فقط في النهار.", "👤", False, "common", "الملاحظة الجيدة والتصويت الصحيح سلاحك الوحيد.", "فوز المواطنين."),
    "نائب الشرطي": Role("نائب الشرطي", "citizens", "يرث دور الشرطي تلقائياً عند موته.", "🚔", False, "common", "ابقَ صامتاً حتى يموت الشرطي.", "فوز المواطنين."),
    "قنّاص": Role("قنّاص", "citizens", "يمتلك رصاصة واحدة يستطيع إطلاقها ليلاً لقتل أي لاعب مباشرة (بدون حماية).", "🎯", True, "legendary", "استخدم رصاصتك على اليقين — لا تضيّعها.", "فوز المواطنين."),
    "مراقب": Role("مراقب", "citizens", "يراقب لاعباً ليلاً: يكشف كم شخص استهدفه في تلك الليلة.", "👁️", True, "rare", "راقب اللاعبين المشبوهين لمعرفة إذا كانوا مستهدَفين.", "فوز المواطنين."),
    "محقق": Role("محقق", "citizens", "يتحقق من لاعبَين في ليلة واحدة: يعرف إذا تفاعلا مع بعضهما.", "🔍", True, "rare", "ابحث عن روابط بين المشتبه بهم.", "فوز المواطنين."),
    "فارس": Role("فارس", "citizens", "إذا استُهدف ليلاً، يقتل مهاجمه ويموت بدوره.", "⚔️", False, "legendary", "أنت شهيد يُقاتل — خاطر بنفسك ليحمي الآخرين.", "فوز المواطنين."),
    "سفير": Role("سفير", "citizens", "يختار لاعباً يحميه من التصويت مرة واحدة طوال اللعبة.", "🤝", True, "legendary", "احمِ الأدوار القوية كالشرطي والطبيب.", "فوز المواطنين."),
    "مجنون": Role("مجنون", "neutral", "هدفه أن يُعدَم بالتصويت. يفوز وحده إذا نُفّذ إعدامه. يخسر إذا قُتل ليلاً.", "🤡", False, "legendary", "استفزّ اللاعبين وتصرّف بشكل مشبوه ليصوّتوا ضدك.", "يفوز إذا نُفّذ إعدامه بالتصويت."),
    "قاتل": Role("قاتل", "killer", "ينتمي لفريق المافيا لكن لا يعرفهم. كل ليلة يختار شخصين ويخمّن دور كلٍ منهما.", "🗡️", True, "legendary", "لا تتسرّع في الانضمام — اجمع المعلومات أولاً.", "فوز المافيا بعد الانضمام."),
}
...
# ============================================================================
# ✅ إصلاح 1: توزيع الأدوار — جدول صحيح
# ============================================================================
...
def distribute_roles(player_ids: list[int], custom_settings: "CustomSettings | None" = None) -> dict[int, Role]:
    ...
    # ✅ جدول توزيع مُصلَح: نسبة مافيا ~ 1:3
    # n=4 → 1 مافيا, n=5 → 1, n=6 → 2, n=7 → 2, n=8 → 2, n=9 → 3, n=10 → 3, n=12 → 4, n=15 → 5
    if n <= 5:
        mafia_count = 1
        helper_count = 0
    elif n <= 7:
    ...
    # ✅ رئيس المافيا فقط في الألعاب الكبيرة (10+) وباحتمال معقول
    if n >= 10 and random.random() < 0.35:
        pool.append(ROLES["رئيس المافيا"])
        pool += [ROLES["مافيا"]] * (mafia_count - 1)
    else:
        pool += [ROLES["مافيا"]] * mafia_count

    # مساعدو المافيا (فقط إذا طلبنا helpers)
    mafia_helpers = ["مضيفة", "ساحرة", "محتال", "جاسوسة", "مزوّر", "مخبر", "محرّض", "عنكبوت", "وحش"]
    random.shuffle(mafia_helpers)
    pool += [ROLES[name] for name in mafia_helpers[:helper_count]]

    # المواطنون الأساسيون دائماً
    ...
    # ✅ المحايدون فقط في الألعاب الكبيرة وبنسب أقل
    neutrals_added = []
    if n >= 9:
        r = random.random()
        if r < 0.3:
            neutrals_added.append(ROLES["قاتل"])
        elif r < 0.5:
            neutrals_added.append(ROLES["مجنون"])
    pool += neutrals_added

    # احسب كم نحتاج من المواطنين الخاصين
    special_count = max(0, n - len(pool))
    pool += [ROLES[name] for name in citizen_specials[:special_count]]

    # أكمل بمواطنين عاديين إذا لزم
    while len(pool) < n:
        pool.append(ROLES["مواطن"])
...
class CustomSettings:
    ...
    fixed_roles: list[str] = field(default_factory=list)
    reveal_role_on_death: bool = False
    mafia_knows_each_other: bool = True
...
def get_stats(user_id: int) -> dict:
    ...
    if key not in stats:
        stats[key] = {
            ...
            "kills_mafia": 0, "saves": 0, "correct_investigations": 0,
            "times_survived": 0, "roles_played": {},
            "win_streak": 0, "max_win_streak": 0,
            "games_as_mafia": 0, "wins_as_mafia": 0,
            "games_as_citizen": 0, "wins_as_citizen": 0,
            "games_as_neutral": 0, "wins_as_neutral": 0,
            "first_blood_count": 0, "night_kills": 0,
        }
...
def _delta_for(player: "PlayerState", winner: str, game: "MafiaGame") -> int:
    ...
    base = 0
    if role.name == "مجنون":
        return 120 if player.killed_by == "vote" else -20
    if role.name == "قاتل":
...
def check_and_grant_achievements(player: "PlayerState", game: "MafiaGame", winner: str, game_stats: dict) -> list[Achievement]:
    uid = player.user.id
    ...
    if won:
        if stats["wins"] == 1: try_grant("first_win")
        if stats["wins"] >= 10: try_grant("win_10")
        if stats["wins"] >= 50: try_grant("win_50")
        if stats["win_streak"] >= 3: try_grant("win_streak_3")
        if stats["win_streak"] >= 5: try_grant("win_streak_5")
    if player.role.name == "مجنون" and player.killed_by == "vote": try_grant("jester_win")
    if player.role.team == "mafia" and won: try_grant("mafia_winner")
    if player.role.team == "citizens" and won: try_grant("citizen_winner")
    if stats["games_played"] >= 10: try_grant("10_games")
    if stats["games_played"] >= 50: try_grant("50_games")
    if stats["games_played"] >= 100: try_grant("100_games")
    if player.alive: try_grant("survivor")
    if player.role.name == "كاهن" and game_stats.get("priest_revived"): try_grant("priest_revived")
    if player.role.name == "جندي" and player.soldier_shield_used: try_grant("soldier_survived")
    if player.role.name == "شهيد" and game_stats.get("martyr_triggered"): try_grant("martyr_revenge")
    if player.role.name == "حارسة" and game_stats.get("guardian_kill"): try_grant("guardian_kill")
    if player.role.name == "قنّاص" and game_stats.get("sniper_mafia_kill"): try_grant("sniper_kill")
    if game.day_count >= 6: try_grant("long_game")
    if game_stats.get(f"first_blood_{uid}"): try_grant("first_blood")
    return granted
...
class PlayerState:
    ...
    ambassador_shield_id: int | None = None
    ...
    forger_fake_role: str | None = None
    heir_id: int | None = None
    is_protected_by_ambassador: bool = False
    spy_reveal_count: int = 0
    cop_mafia_found: int = 0
...
class NightActions:
    ...
    sniper_target: int | None = None
    watcher_target: int | None = None
    detective_pair: tuple[int, int] | None = None
    forger_target: int | None = None
    forger_fake_role: str | None = None
    informant_target: int | None = None
    spider_trap: int | None = None
    inciter_target: int | None = None
    inciter_voter: int | None = None
    heir_activated: bool = False
    mafia_boss_heir: int | None = None


class MafiaGame:
    def __init__(self, guild: discord.Guild, channel: discord.TextChannel, mode: str = MODE_NORMAL, settings: CustomSettings | None = None):
        self.guild = guild
    ...
    # ✅ إصلاح 2: alive_mafia لا تشمل القاتل إلا بعد انضمامه
    def alive_mafia(self) -> list[PlayerState]:
        result = []
        for p in self.alive_players():
            if p.role.team == "mafia":
                result.append(p)
            elif p.role.name == "قاتل" and p.joined_mafia:
                result.append(p)
        return result
    ...
    def mafia_team_members(self, *, alive_only: bool = True) -> list[PlayerState]:
        ...
        return [p for p in pool if p.role.team == "mafia" or (p.role.name == "قاتل" and p.joined_mafia)]
    ...
    # ✅ إصلاح 3: منطق check_winner مُصلَح بالكامل
    def check_winner(self) -> str | None:
        alive = self.alive_players()

        # لا يوجد لاعبون → المواطنون يفوزون (حالة نادرة)
        if not alive:
            return "citizens"

        mafia_alive = self.alive_mafia()
        mafia_count = len(mafia_alive)

        # المواطنون الفعليون (غير المافيا وغير المحايدين المنفصلين)
        citizens_alive = [
            p for p in alive
            if p.role.team == "citizens"
            or (p.role.name == "قاتل" and not p.joined_mafia)
            or p.role.name == "مجنون"
        ]
        citizen_count = len(citizens_alive)

        # السياسي له وزن إضافي في الإجماع
        has_politician = any(p.role.name == "سياسي" for p in citizens_alive)
        effective_citizens = citizen_count + (1 if has_politician else 0)

        # ✅ لا مافيا → المواطنون يفوزون
        if mafia_count == 0:
            return "citizens"

        # ✅ المافيا تساوي أو تتجاوز المواطنين الفعليين (وهناك مواطنون)
        # يجب أن يكون هناك مواطنون فعليون حتى تنتهي اللعبة
        if mafia_count >= effective_citizens and citizen_count > 0:
            return "mafia"

        # ✅ إذا لا يوجد مواطنون أصلاً (كل الأحياء مافيا)
        if citizen_count == 0 and mafia_count > 0:
            return "mafia"
...
async def mute_player(game: MafiaGame, uid: int, reason: str = "") -> None:
    ...
    try:
        await game.channel.set_permissions(member, overwrite=new_ow, reason=reason or "كتم")
    except discord.HTTPException:
        pass
...
def _pick_morning_event(events: set[str]) -> str:
    priority = ["jester_win", "killer_success", "sniper_kill", "guardian_kill",
...
def role_image_path(role_name: str) -> Path:
    ...
    return Path(__file__).parent / "attached_assets" / f"role_{safe}.png"


async def send_with_optional_image(dest, embed: discord.Embed, event: str | None = None, **kwargs) -> discord.Message:
    if event:
...
def build_role_embed(role: Role) -> discord.Embed:
    ...
    team_name = {"mafia": "🔴 فريق المافيا", "citizens": "🟢 فريق المواطنين", "neutral": "🟣 محايد", "killer": "⚫ قاتل سري"}.get(role.team, role.team)
    rarity_display = {"common": "⚪ شائع", "rare": "🔵 نادر", "legendary": "🟡 أسطوري"}.get(role.rarity, role.rarity)
    ...
    embed.set_footer(text="مافيا 42 — النسخة 3.1")
    return embed
...
class SettingsView(discord.ui.View):
    ...
    async def mode_fast(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message("✅ تم ضبط الوضع السريع.", ephemeral=True)
...
class LobbyView(discord.ui.View):
    ...
    def _build_embed(self) -> discord.Embed:
        ...
        mode_name = {MODE_NORMAL: "🎮 عادي", MODE_FAST: "⚡ سريع", MODE_RANKED: "🏆 مصنّف", MODE_CUSTOM: "🔧 مخصص"}.get(self.game.mode, self.game.mode)
        embed = discord.Embed(
            title="🕵️‍♂️ غرفة انتظار مافيا 42",
            description=(f"👑 **المنشئ:** <@{self.host_id}>\n🎮 **الوضع:** {mode_name}\n"
                         f"👥 **الحد الأدنى:** {MIN_PLAYERS} لاعبين | الأقصى: {MAX_PLAYERS}\n\n"
                         f"**اللاعبون ({len(ids)}/{MAX_PLAYERS}):**\n{names}"),
            color=discord.Color.gold(),
        )
        if self.game.spectators:
            embed.add_field(name="👁️ مراقبون", value=" • ".join(f"<@{s}>" for s in self.game.spectators), inline=False)
    ...
    async def join(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.game.add_lobby_player(uid):
            ...
            await interaction.response.send_message(f"انضممت! 🎉\nنقاطك: **{pts}** ({rank})", ephemeral=True)
    ...
    async def start(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.host_id:
            return await interaction.response.send_message(f"فقط <@{self.host_id}> يستطيع البدء.", ephemeral=True)
        if len(self.game.lobby_user_ids) < MIN_PLAYERS:
            return await interaction.response.send_message(
                f"يجب {MIN_PLAYERS} لاعبين على الأقل (حالياً {len(self.game.lobby_user_ids)}).", ephemeral=True)
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.host_id:
            return await interaction.response.send_message(f"فقط <@{self.host_id}> يستطيع الإلغاء.", ephemeral=True)
        await interaction.response.defer()
        for child in self.children:
            child.disabled = True
        if interaction.message:
...
class _RoleTargetSelect(discord.ui.Select):
    def __init__(self, candidates: list[PlayerState], placeholder: str, callback):
        opts = _player_options(candidates) or [discord.SelectOption(label="لا يوجد هدف متاح", value="none")]
...
def _make_action_view(candidates: list[PlayerState], placeholder: str, callback, *, allow_skip: bool = True, timeout: float | None = None) -> discord.ui.View:
...
class _MultiTargetSelect(discord.ui.Select):
    def __init__(self, candidates: list[PlayerState], placeholder: str, callback, count: int):
        opts = _player_options(candidates) or [discord.SelectOption(label="لا يوجد", value="none")]
        super().__init__(placeholder=placeholder, min_values=count, max_values=count if len(opts) >= count else len(opts), options=opts[:25])
        self._callback = callback
...
def _make_multi_action_view(candidates: list[PlayerState], placeholder: str, callback, count: int, *, allow_skip: bool = True) -> discord.ui.View:
...
class _KillerView(discord.ui.View):
    def __init__(self, candidates: list[PlayerState], role_names: list[str], on_submit):
        ...
        player_opts = _player_options(candidates)[:25] or [discord.SelectOption(label="لا يوجد", value="none")]
        role_opts = [discord.SelectOption(label=f"{ROLES[r].emoji} {r}", value=r) for r in role_names if r in ROLES][:25]
...
class _ForgerView(discord.ui.View):
    def __init__(self, candidates: list[PlayerState], on_submit):
        ...
        role_opts = [discord.SelectOption(label=f"{r.emoji} {r.name}", value=r.name) for r in ROLES.values() if r.team == "citizens"][:25]
...
def _build_night_menu(game: MafiaGame, player: PlayerState) -> tuple[discord.Embed, discord.ui.View | None]:
    ...
    if role.name in ("مافيا", "رئيس المافيا"):
        ...
        if role.name == "رئيس المافيا" and not player.heir_id:
            extra_desc = "\n\n⚠️ **لم تختر وريثاً بعد!** استخدم &وريث @لاعب"
        embed = discord.Embed(title="🔪 وقت المافيا", description=f"اختر الضحية. الأكثر تصويتاً يُقتل.{extra_desc}", color=discord.Color.dark_red())
        team = game.mafia_team_members(alive_only=True)
        if team:
            embed.add_field(name="👥 فريقك", value="\n".join(f"• {p.user.display_name} ({p.role.emoji} {p.role.name})" for p in team), inline=False)
    ...
    if role.name == "وحش":
        async def cb_beast(interaction, target_id):
            ...
            msg = "لن تبحث الليلة." if target_id is None else f"👹 ستبحث في **{game.get(target_id).user.display_name}**."
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="👹 وقت الوحش", description="اختر لاعباً للبحث عن المافيا فيه.", color=discord.Color.dark_red()), _make_action_view(candidates, "اختر هدفاً", cb_beast, timeout=night_sec + 15)
    ...
    if role.name == "مضيفة":
        async def cb_hostess(interaction, target_id):
            ...
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="💋 وقت المضيفة", description="اختاري لاعباً لمنعه من تنفيذ دوره.", color=discord.Color.magenta()), _make_action_view(candidates, "اختاري هدفاً", cb_hostess, timeout=night_sec + 15)
    ...
    if role.name == "ساحرة":
        ...
        async def cb_witch(interaction, target_id):
            ...
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="🧙‍♀️ وقت الساحرة", description="اختاري مواطناً لتعطيل قدرته.", color=discord.Color.purple()), _make_action_view(non_mafia, "اختاري من تسحرين", cb_witch, timeout=night_sec + 15)

    if role.name == "جاسوسة":
        async def cb_spy(interaction, target_id):
            na.spy_target = (player.user.id, target_id) if target_id else None
            msg = "لن تتجسسي." if not target_id else "🕵️‍♀️ النتيجة في الصباح."
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="🕵️‍♀️ وقت الجاسوسة", description="اختاري لاعباً لكشف دوره.", color=discord.Color.dark_red()), _make_action_view(candidates, "اختاري هدف التجسس", cb_spy, timeout=night_sec + 15)

    if role.name == "مزوّر":
        async def cb_forger(interaction, target_id, fake_role_name):
            na.forger_target = target_id
            na.forger_fake_role = fake_role_name
            msg = "لن تزوّر الليلة." if not target_id else f"🖊️ ستزوّر دور **{game.get(target_id).user.display_name}**."
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="🖊️ وقت المزوّر", description="اختر لاعباً وزوّر دوره.", color=discord.Color.dark_orange()), _ForgerView(candidates, cb_forger)

    if role.name == "مخبر":
        async def cb_informant(interaction, target_id):
            na.informant_target = target_id
            msg = "لن تستخدم قدرتك." if not target_id else f"📋 ستعكس نتيجة الشرطي عن **{game.get(target_id).user.display_name}**."
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="📋 وقت المخبر", description="اختر لاعباً: إذا حقّق الشرطي معه الليلة، تُعكس النتيجة.", color=discord.Color.dark_red()), _make_action_view(candidates, "اختر الهدف", cb_informant, timeout=night_sec + 15)
    ...
    if role.name == "عنكبوت":
        async def cb_spider(interaction, target_id):
            ...
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="🕷️ وقت العنكبوت", description="انصب فخاً على لاعب.", color=discord.Color.dark_red()), _make_action_view(candidates, "اختر من تنصب عليه", cb_spider, timeout=night_sec + 15)

    if role.name == "محرّض":
        async def cb_inciter(interaction, target_id):
            na.inciter_voter = target_id
            msg = "لن تحرّض أحداً." if not target_id else f"📢 ستجبر **{game.get(target_id).user.display_name}** على التصويت ضد هدفك."
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="📢 وقت المحرّض", description="اختر لاعباً لإجباره على التصويت ضد هدف تختاره في النهار.", color=discord.Color.dark_red()), _make_action_view(candidates, "اختر من تحرّض", cb_inciter, timeout=night_sec + 15)
    ...
    if role.name == "شرطي":
        async def cb_cop(interaction, target_id):
            ...
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="🚓 وقت الشرطي", description="اختر لاعباً للتحقيق معه.", color=discord.Color.blue()), _make_action_view(candidates, "اختر من تحقق معه", cb_cop, timeout=night_sec + 15)

    if role.name == "نائب الشرطي":
        return discord.Embed(title="🚔 نائب الشرطي", description="ستأخذ دور الشرطي تلقائياً عند وفاته. لا تنفيذ الآن.", color=discord.Color.blue()), None
    ...
    if role.name == "طبيب":
        ...
        async def cb_doctor(interaction, target_id):
            ...
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="💉 وقت الطبيب", description="اختر لاعباً لحمايته (يشمل نفسك).", color=discord.Color.green()), _make_action_view(all_alive, "اختر من تحمي", cb_doctor, timeout=night_sec + 15)
    ...
    if role.name == "حارسة":
        async def cb_guardian(interaction, target_id):
            ...
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="🛡️ وقت الحارسة", description="إذا هاجمت المافيا من تحرسينه، تقتلين المهاجم.", color=discord.Color.dark_teal()), _make_action_view(candidates, "اختاري من تحرسين", cb_guardian, timeout=night_sec + 15)

    if role.name == "عميل سري":
        na.secret_agent = player.user.id
        if game.day_count < 2:
            return discord.Embed(title="🕴️ عميل سري", description="تبدأ من الليلة الثانية. انتظر.", color=discord.Color.dark_blue()), None
        return discord.Embed(title="🕴️ عميل سري", description="ستكشف دور مواطن عشوائي تلقائياً في الصباح.", color=discord.Color.dark_blue()), None

    if role.name == "كاهن":
        dead = game.dead_players()
        if not dead:
            return discord.Embed(title="⛪ كاهن", description="لا يوجد موتى.", color=discord.Color.gold()), None
        async def cb_priest(interaction, target_id):
            if not target_id:
                ...
                msg = f"⛪ ستعيد **{game.get(target_id).user.display_name}**."
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="⛪ وقت الكاهن", description="اختر ميتاً لإعادته — مرة واحدة.", color=discord.Color.gold()), _make_action_view(dead, "اختر من تعيد", cb_priest, timeout=night_sec + 15)

    if role.name == "عرافة":
        dead = game.dead_players()
        if not dead:
            return discord.Embed(title="🔮 عرافة", description="لا يوجد موتى.", color=discord.Color.dark_purple()), None
        async def cb_oracle(interaction, target_id):
            ...
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="🔮 وقت العرافة", description="اختاري ميتاً لمعرفة دوره.", color=discord.Color.dark_purple()), _make_action_view(dead, "اختاري ميتاً", cb_oracle, timeout=night_sec + 15)

    if role.name == "مراسلة":
        if game.day_count < 2:
            return discord.Embed(title="📰 مراسلة", description="لا يمكنك النشر في الليلة الأولى.", color=discord.Color.gold()), None
        async def cb_journalist(interaction, target_id):
            if not target_id:
                ...
                msg = f"📰 ستنشرين دور **{game.get(target_id).user.display_name}**."
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="📰 وقت المراسلة", description="انشري دور لاعب علناً — مرة واحدة.", color=discord.Color.gold()), _make_action_view(candidates, "اختاري من تنشرين", cb_journalist, timeout=night_sec + 15)
    ...
    if role.name == "رجل عصابة":
        async def cb_gangster(interaction, target_id):
            ...
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="🚫 وقت رجل العصابة", description="اختر لاعباً لمنعه من التصويت في النهار التالي.", color=discord.Color.dark_gold()), _make_action_view(candidates, "اختر من تمنع", cb_gangster, timeout=night_sec + 15)

    if role.name == "قنّاص":
        async def cb_sniper(interaction, target_id):
            na.sniper_target = target_id
            msg = "احتفظت برصاصتك." if not target_id else f"🎯 ستطلق على **{game.get(target_id).user.display_name}**."
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="🎯 وقت القنّاص", description="امتلك رصاصة واحدة تقتل أي لاعب مباشرة.\nاستخدمها بحكمة!", color=discord.Color.dark_green()), _make_action_view(candidates, "اختر هدفك", cb_sniper, timeout=night_sec + 15)
    ...
    if role.name == "مراقب":
        async def cb_watcher(interaction, target_id):
            ...
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="👁️ وقت المراقب", description="اختر لاعباً لمراقبته.", color=discord.Color.teal()), _make_action_view(candidates, "اختر من تراقب", cb_watcher, timeout=night_sec + 15)

    if role.name == "محقق":
        if len(candidates) < 2:
            return discord.Embed(title="🔍 محقق", description="لا يوجد لاعبان كافيان.", color=discord.Color.blue()), None
        async def cb_detective(interaction, ids: list[int]):
            if len(ids) >= 2:
                ...
                msg = "تخطيت."
            await interaction.response.send_message(msg, ephemeral=True)
        return discord.Embed(title="🔍 وقت المحقق", description="اختر لاعبَين: ستعرف إذا كان أحدهما استهدف الآخر.", color=discord.Color.blue()), _make_multi_action_view(candidates, "اختر لاعبَين", cb_detective, 2)
    ...
    if role.name == "سفير":
        ...
        async def cb_ambassador(interaction, target_id):
            ...
            if tp:
                ...
                await interaction.response.send_message(f"🤝 منحت **{tp.user.display_name}** حصانة.", ephemeral=True)
        return discord.Embed(title="🤝 وقت السفير", description="منح لاعب حصانة من الإعدام بالتصويت مرة واحدة.", color=discord.Color.teal()), _make_action_view(ambassadored or candidates, "اختر المحمي", cb_ambassador, allow_skip=True, timeout=night_sec + 15)
    ...
    if role.name == "قاتل":
        if player.joined_mafia:
            ...
            async def cb_join(interaction, target_id):
                if not target_id:
                    ...
                    msg = f"🔪 صوّتت لقتل **{game.get(target_id).user.display_name}**."
                await interaction.response.send_message(msg, ephemeral=True)
            return discord.Embed(title="🗡️🔪 قاتل (انضممت للمافيا)", description="صوّت مع المافيا.", color=discord.Color.dark_red()), _make_action_view(mafia_targets, "اختر ضحيتك", cb_join, timeout=night_sec + 15)

        killer_candidates = [p for p in candidates if p.role.name != "طبيب"]
        guessable_roles = [r for r in ROLES if ROLES[r].team in ("citizens", "mafia") and r != "قاتل"]
        ...
        async def on_killer_submit(interaction, t1_id, r1_name, t2_id, r2_name):
            ...
            await interaction.response.send_message(f"🗡️ اخترت:\n• **{n1}** → {r1_name}\n• **{n2}** → {r2_name}", ephemeral=True)

        return discord.Embed(
            title="🗡️ وقت القاتل",
            description="أنت من **فريق المافيا** لكن لا تعرفهم.\nاختر **شخصَين** وخمّن دور كل واحد.",
...
class _NightRoleView(discord.ui.View):
    ...
    async def spectator_view(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        lines = [f"🟢 {p.user.display_name}" for p in alive]
        await interaction.response.send_message("👁️ **اللاعبون الأحياء:**\n" + "\n".join(lines), ephemeral=True)
...
class DiscussionView(discord.ui.View):
    ...
    async def roles_list(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        desc = f"**أحياء ({len(alive)}):**\n" + " • ".join(p.user.mention for p in alive)
        if dead:
            reveal = self.game.settings.reveal_role_on_death
            desc += f"\n\n**أموات ({len(dead)}):**\n"
            desc += "\n".join(f"💀 {p.user.display_name}" + (f" — {p.role.emoji} {p.role.name}" if reveal else "") for p in dead)
        await interaction.response.send_message(embed=discord.Embed(title="📋 حالة اللعبة", description=desc, color=discord.Color.blurple()), ephemeral=True)
...
# ============================================================================
# ✅ إصلاح 4: save_game_to_history مُعرَّفة
# ============================================================================
...
async def run_game(game: MafiaGame):
    ...
    try:
        ...
        # ✅ إصلاح 5: التحقق من عدد المافيا قبل البدء
        actual_mafia = len(game.alive_mafia())
        actual_citizens = len(game.alive_citizens())
        log.info("توزيع الأدوار: %d مافيا، %d مواطن، %d إجمالي", actual_mafia, actual_citizens, len(game.players))

        if actual_mafia == 0:
            await game.channel.send("❌ خطأ في التوزيع: لا يوجد مافيا. أعد المحاولة.")
        ...
        # ✅ إصلاح 6: إظهار عدد المافيا والمواطنين (بدون كشف من هم)
        ...
        # ✅ إصلاح 7: لا نتحقق من الفائز قبل الليلة الأولى
        while True:
...
# ✅ إصلاح 8: _update_player_stats_after_game تحسب الفائز من game_stats لا check_winner
async def _update_player_stats_after_game(game: MafiaGame):
    winner = game.game_stats.get("final_winner", "unknown")
    for p in game.players.values():
        uid = p.user.id
        stats = get_stats(uid)
        ...
        if p.role.team == "mafia":
            ...
            if won: stats["wins_as_mafia"] += 1
        elif p.role.team == "citizens":
            stats["games_as_citizen"] += 1
            if won: stats["wins_as_citizen"] += 1
        else:
            stats["games_as_neutral"] += 1
            if won: stats["wins_as_neutral"] += 1
...
async def announce_winner(game: MafiaGame, winner: str):
    ...
    # ✅ إصلاح 9: حفظ الفائز في game_stats
    actual_winner = "citizens" if winner == "jester" else winner
    game.game_stats["final_winner"] = actual_winner

    titles = {"citizens": "🏆 فوز فريق المواطنين!", "mafia": "🔪 فوز فريق المافيا!", "jester": "🤡 فوز المجنون!"}
    colors = {"citizens": discord.Color.green(), "mafia": discord.Color.dark_red(), "jester": discord.Color.purple()}
    ...
    embed.add_field(name="⏱️ مدة اللعبة", value=f"الليالي: **{game.day_count}** | الوقت: **{game.elapsed_time()}**", inline=False)
    ...
    event = {"citizens": "win_citizens", "mafia": "win_mafia", "jester": "jester_win"}.get(winner, "quiet")
...
# ============================================================================
# ✅ إصلاح 10: resolve_night — الممرضة ونائب الشرطي يرثان فقط عند موت حامل الدور في نفس الليلة
# ============================================================================
...
async def resolve_night(game: MafiaGame) -> tuple[list[str], set[str]]:
    ...
    # سجّل اللاعبين الأحياء قبل الليلة
    alive_before = {uid for uid, p in game.players.items() if p.alive}
    ...
    if na.spider_trap is not None:
        ...
        if spider and not is_blocked(spider.user.id):
            ...
            if na.mafia_votes and trap_target in na.mafia_votes.values():
                for mid, tid in na.mafia_votes.items():
                    if tid == trap_target:
                        mp = game.get(mid)
                        if mp:
            ...
            trap_name = game.get(trap_target)
            if attackers:
                spider.pending_notices.append(
                    f"🕷️ **فخّك أمسك!** من استهدف {trap_name.user.display_name if trap_name else '؟'}:\n"
    ...
    if na.watcher_target is not None:
        ...
        if watcher and not is_blocked(watcher.user.id):
            ...
            targeting_count = 0
            if na.mafia_votes and wt in na.mafia_votes.values():
            ...
            if na.spy_target and na.spy_target[1] == wt:
                targeting_count += 1
            wt_player = game.get(wt)
            watcher.pending_notices.append(
                f"👁️ **نتيجة المراقبة على {wt_player.user.display_name if wt_player else '؟'}:**\n"
    ...
    if na.detective_pair is not None:
        ...
        if detective and not is_blocked(detective.user.id):
            ...
            if a and b:
                ...
                detective.pending_notices.append(f"🔍 **نتيجة التحقيق** بين **{a.user.display_name}** و **{b.user.display_name}**:\n{result}")
    ...
    if beast and not is_blocked(beast.user.id) and na.beast_target:
        ...
        mafia_main_alive = any(p.role.name in ("مافيا", "رئيس المافيا") for p in game.alive_players())
        if pick and pick.role.team == "mafia":
            beast_pierces = True
            beast.pending_notices.append(f"👹 وجدت مافيا في **{pick.user.display_name}** — ضربتهم تخترق الحماية!")
        else:
            beast.pending_notices.append(f"👹 لم تجد مافيا في **{pick.user.display_name if pick else '؟'}**." + ("" if mafia_main_alive else " ستقتله بنفسك."))
    ...
    if mafia_target is not None:
        attacker = next((p for p in game.alive_mafia() if p.role.name in ("مافيا", "رئيس المافيا")), None)
    ...
    for uid in list(died_this_night):
        victim = game.get(uid)
        if victim and victim.role.name == "شهيد" and victim.killed_by_player:
            ...
            if killer_p and killer_p.alive:
                ...
                died_this_night.add(killer_p.user.id)
    ...
    if na.priest_revive is not None:
        ...
        if priest and not is_blocked(priest.user.id) and not priest.priest_used:
            ...
            if target and not target.alive:
                ...
                # إزالة من قائمة الموتى في هذه الليلة
                died_this_night.discard(target.user.id)
                log_lines.append(f"⛪ الكاهن أعاد {target.user.mention} للحياة!")
                events.add("priest_revive")
                await unmute_player(game, target.user.id)

    # ✅ إصلاح 11: الممرضة ترث فقط إذا مات الطبيب في هذه الليلة
    if na.doctor_save is not None or True:  # تحقق دائماً
        for uid in died_this_night:
            dead_p = game.get(uid)
            if dead_p and dead_p.role.name == "طبيب":
                nurse = game.alive_by_role("ممرضة")
                if nurse:
                    nurse.role = ROLES["طبيب"]
                    nurse.pending_notices.append("🏥 **ورثتِ دور الطبيب!** ابدئي بحماية اللاعبين.")
                    log_lines.append("🏥 الممرضة ورثت دور الطبيب!")
                break

    # ✅ إصلاح 12: نائب الشرطي يرث فقط إذا مات الشرطي في هذه الليلة
    for uid in died_this_night:
        dead_p = game.get(uid)
        if dead_p and dead_p.role.name == "شرطي":
    ...
    if na.journalist_reveal is not None:
        ...
        if journalist and not is_blocked(journalist.user.id) and not journalist.journalist_used:
            ...
            if target:
                ...
                log_lines.append(f"📰 **خبر عاجل**: {target.user.mention} هو {r_obj.emoji} **{role_shown}**!")
    ...
    if killer and na.killer_guesses:
        if is_blocked(killer.user.id):
            ...
            if mafia_picks:
                ...
                killer.pending_notices.append(f"🗡️ ✅ اكتشفت المافيا — انضممت لهم!\nزملاؤك: {mate_names or 'لا أحد'}")
                for m in game.mafia_team_members(alive_only=True):
                    if m.user.id != killer.user.id:
                        m.pending_notices.append(f"🗡️ القاتل **{killer.user.display_name}** انضم للفريق.")
            elif len(citizen_picks) >= 2:
                all_correct = all(na.killer_role_guesses.get(p.user.id) == p.role.name for p in citizen_picks)
                if all_correct:
    ...
    for uid in died_this_night:
        dead_p = game.get(uid)
        if dead_p and dead_p.role.name == "رئيس المافيا" and dead_p.heir_id and not na.heir_activated:
            heir = game.get(dead_p.heir_id)
            if heir and heir.alive and heir.role.name != "مافيا":
    ...
    if not log_lines or not any("قُتل" in l or "أنقذ" in l or "صدّ" in l for l in log_lines):
    ...
    if na.cop_target:
        ...
        if cop and target:
            if cop_id in blocked:
                ...
                if "مافيا" in verdict and target.role.team == "mafia":
                    cop.cop_mafia_found += 1
    ...
    if na.spy_target:
        ...
        if spy and target:
            if spy_id in blocked:
                ...
                role_shown = target.forger_fake_role or target.role.name
                r_obj = ROLES.get(role_shown, target.role)
                spy.pending_notices.append(f"🕵️‍♀️ دور **{target.user.display_name}** هو: {r_obj.emoji} **{role_shown}**")
                spy.spy_reveal_count += 1
                for mate in game.alive_mafia():
                    if mate.user.id == spy_id:
                        continue
                    mate.pending_notices.append(f"🕵️‍♀️ كشفت الجاسوسة دور **{target.user.display_name}**: {r_obj.emoji} **{role_shown}**")
    ...
    if na.secret_agent and game.day_count >= 2:
        ...
        if agent:
            if na.secret_agent in blocked:
                ...
                citizens = [p for p in game.alive_citizens() if p.user.id != agent.user.id]
                if citizens:
                    pick = random.choice(citizens)
                    role_shown = pick.forger_fake_role or pick.role.name
                    r_obj = ROLES.get(role_shown, pick.role)
                    agent.pending_notices.append(f"🕴️ كشفت أن **{pick.user.display_name}** هو {r_obj.emoji} **{role_shown}**")
                else:
    ...
    if na.oracle_target:
        ...
        if oracle and dead:
            if oracle_id in blocked:
                ...
                oracle.pending_notices.append(f"🔮 **{dead.user.display_name}** كان دوره: {r_obj.emoji} **{role_shown}**")


# ============================================================================
# ✅ إصلاح 14: run_day — المحرّض يُطبَّق صحيحاً
# ============================================================================
...
async def run_day(game: MafiaGame):
    ...
    for voter_id, target_id in game.day_votes.items():
        ...
        # ✅ إصلاح: المحرّض يُجبر المصوّت على التصويت لهدف المحرّض
        na = game.night_actions
        if na.inciter_voter == voter_id and na.inciter_target is not None:
            # تجاهل صوته الأصلي واحسب لهدف المحرّض
            forced_target = game.get(na.inciter_target)
            if forced_target and forced_target.alive:
                counts[na.inciter_target] += weight
                continue
    ...
    # ✅ إصلاح 15: تحقق من الفوز بعد الإعدام فوراً
    winner = game.check_winner()
    if winner:
        await announce_winner(game, winner)
...
async def cmd_start(ctx: commands.Context, mode: str = ""):
    ...
    if not is_channel_allowed(ctx.guild.id, ctx.channel.id):
        ...
        if not allowed:
            return await ctx.send("⚠️ **لم تُضف قناة للعبة بعد.**\nالمسؤول يضيف قناة عبر: `&اضافه_قناة <ID>`")
        return await ctx.send("❌ **هذه القناة غير مضافة للعبة.**\nاطلب من المسؤول إضافتها.")
...
async def cmd_whisper(ctx: commands.Context, *, message: str = ""):
    ...
    for mate in team:
        ...
        try:
            ...
            await dm.send(f"🔪 **رسالة سرية من {player.user.display_name}:**\n{message}")
        except discord.HTTPException:
            pass
    await ctx.send(f"🔪 **رسالة سرية من {player.user.mention}:** (وصلت لزملائك في المافيا)", delete_after=10)
...
async def cmd_heir(ctx: commands.Context, member: discord.Member | None = None):
    ...
    try:
        ...
        pass
    await ctx.send("👑 تم تعيين وريثك سراً.", delete_after=5)
...
async def cmd_list_channels(ctx: commands.Context):
    ...
    await ctx.send(embed=discord.Embed(title="🎮 قنوات لعبة المافيا", description="\n".join(lines), color=discord.Color.blurple()))
...
async def cmd_status(ctx: commands.Context):
    ...
    alive = game.alive_players()
    dead = game.dead_players()
    phase_names = {"waiting": "⏳ انتظار", "night": "🌙 ليل", "day": "☀️ نهار", "ended": "🏁 انتهت"}
    embed = discord.Embed(title="📊 حالة اللعبة", color=discord.Color.blurple())
...
async def cmd_points(ctx: commands.Context, member: discord.Member | None = None):
    ...
    embed = discord.Embed(title=f"🏅 ملف اللاعب: {target.display_name}", color=discord.Color.gold())
    ...
    if roles_played:
        ...
        embed.add_field(name="⭐ أكثر دور لعبه", value=f"{r.emoji if r else ''} {top_role} ({roles_played[top_role]} مرة)", inline=False)
    await ctx.send(embed=embed)
...
async def cmd_leaderboard(ctx: commands.Context):
    ...
    embed = discord.Embed(title="🏆 أفضل اللاعبين", description="\n".join(lines), color=discord.Color.gold())
...
async def cmd_backup(ctx: commands.Context):
    files = []
    for path in (RANKS_FILE, ALLOWED_CHANNELS_FILE, STATS_FILE):
        if path.exists():
...
async def cmd_roles(ctx: commands.Context):
    ...
    embed.add_field(name="🔴 المافيا", value="\n".join(f"{r.emoji} **{n}** {'⭐' if r.rarity=='legendary' else '🔵' if r.rarity=='rare' else ''}" for n, r in mafia_roles), inline=True)
    embed.add_field(name="🟢 المواطنون", value="\n".join(f"{r.emoji} **{n}** {'⭐' if r.rarity=='legendary' else '🔵' if r.rarity=='rare' else ''}" for n, r in citizen_roles), inline=True)
    embed.add_field(name="🟣 محايدون", value="\n".join(f"{r.emoji} **{n}** ⭐" for n, r in neutral_roles) or "—", inline=True)
...
async def cmd_role_info(ctx: commands.Context, *, role_name: str = ""):
    ...
    for name, role in ROLES.items():
        if role_name.strip() in name or name in role_name.strip():
            ...
            break
    if not found:
        return await ctx.send(f"لم أجد دوراً باسم **{role_name}**.")
...
async def cmd_help(ctx: commands.Context, section: str = ""):
    ...
    embed = discord.Embed(
        ...
        description="بوت لعبة المافيا الاحترافية بـ 31 دور.",
    ...
    embed.add_field(name="🔧 أوامر المسؤول", value=(
        "`&اضافه_قناة <ID>` — إضافة قناة للعبة\n"
        "`&حذف_قناة <ID>` — حذف قناة\n"
        "`&قنوات` — عرض القنوات\n"
        "`&ريست_نقاط` — إعادة ضبط النقاط\n"
...
async def cmd_bot_info(ctx: commands.Context):
    embed = discord.Embed(title=f"🤖 مافيا 42 — النسخة {BOT_VERSION}", description="بوت لعبة المافيا الاحترافي مع 31 دور، نظام إنجازات، وإحصائيات مفصّلة.", color=discord.Color.blurple())
